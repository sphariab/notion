...
const Form = ({ mode, saveNote, title = '', text = '', id }) => {
    ...
    const enteredSymbolsLength = inputEl && inputEl.current && inputEl.current.props.value.length;
    ...
    return (
        <Col span={10}>
            <Row className='field'>
                ...
                { isWarning && <>
                    <Alert className='field__alert' banner type='warning' message='max length is 20 symbols' closable /></>}
                <span className={isMaxTitleLengthExceeded ? 'counter-red': 'counter-green' }>{titleMaxLength - enteredSymbolsLength}</span>
            </Row>
            <Row className='field'>
                <div className='field__title'>Title max Length</div>
                <InputNumber onChange={value => setMaxLength(value)} defaultValue={20} />
            </Row>
            <Row className='field'>
